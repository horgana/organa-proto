...
namespace Organa
{
    ...
    [BurstCompile]
    [Serializable]
    public struct NoiseProducer2D<N> : IDisposable, INoiseProcessor2D<float> 
        where N : struct, INoiseSource<float2, float>
    {
        ...
        public NoiseProducer2D(NoiseProfile profile, int count = 0, Allocator allocator = Allocator.None)
        {
        ...
        {
            [ReadOnly] public NoiseProducer2D<N> GeneratorJob;
